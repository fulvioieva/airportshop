...
public class TicketService implements TicketFunctions {
	...
	public boolean buyTicket(long userId, long flightId, int qta, PaymentType paymentType) throws Exception {
		...
		if (!userService.ifUserIsLogged(userId)) { throw new Exception(); }
		...
		// Potresti aggiungere anche il controllo del PaymentType, ma dovresti riceverlo in tipo stringa, non PaymentType
	...
	public boolean addTicketQta(long ticketId, int qta, long userId) throws Exception {
		// Controllo se esiste l'userId (e se loggato) e il biglietto, poi prendo l'id del volo
		if (!userRepository.existsById(userId)) { throw new Exception(); }
		if (!userService.ifUserIsLogged(userId)) { throw new Exception(); }
		if (!ticketRepository.existsById(ticketId)) { throw new Exception(); }
	...
	public boolean removeTicketQta(long ticketId, int qta, long userId) throws Exception {
		// Controllo se esiste l'userId (e se loggato) e il biglietto, poi prendo il biglietto
		if (!userRepository.existsById(userId)) { throw new Exception(); }
		if (!userService.ifUserIsLogged(userId)) { throw new Exception(); }
		if (!ticketRepository.existsById(ticketId)) { throw new Exception(); }
		
		Ticket ticket = ticketRepository.findByIdAndUserId(ticketId, userId).get();
